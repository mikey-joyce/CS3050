...
void findOutdegrees(FILE *, Graph *, int *);
...
void findSCC(Graph *graphData1, Graph *graphData2, int vertices, int *myStack, int stackCounter)
{
    ...
    int *indexes = malloc(sizeof(int) * (vertices + 1));
    while (stackCounter != -1)
    {
        ...
        {
            printf("SCC %d: \n", count++);
            DFS(graphData2, vertices, visited, indexes, count);
            printf("\n");
        }
    }
}
...
void DFS(Graph *myGraph, int num, bool visited[], int *indexes, int count)
{
    indexes[num] = count;
    ...
    while (currentHead)
    {
        ...
        {
            //call DFS
            DFS(myGraph, currentHead->destination, visited, indexes, count);
        }
...
void linkSCCs(int source, int SCCfound, Graph *outDegreeGraph)
{
    AdjListNode *hold = outDegreeGraph->myArray[source].head;

    while (hold)
    {
        // if connection previously found, keep going
        if (hold->destination == SCCfound)
        {
            return;
        }
        else
        {
            // keep moving if not found
            hold = hold->next;
        }
    }

    AdjListNode *myNewNode = createNode(SCCfound);

    myNewNode->next = outDegreeGraph->myArray[source].head;
    outDegreeGraph->myArray[source].head = myNewNode;
}

void findOutdegrees(FILE *fPtr, Graph *outDeg, int index[])
{
    int v1, v2;

    while (fscanf(fPtr, "%d %d", &v1, &v2))
    {
        if (index[v1] != index[v2])
        {
            linkSCCs(index[v1], index[v2], outDeg);
        }
    }
}
